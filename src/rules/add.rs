...
pub fn add() -> Vec<Rewrite> {
    vec![
        ...
        // rw!("sub-const-denom-2"; "( - ( / ( + ?x ?c1 ) ?c0 ) ( / ( + ?x ?y ) ?c0 ) )" => "( / ( - ( - (- ( + ?c0 ?c1 ) num1 ) ?y ) ( % ( + ?x ( % ?c1 ?c0 ) ) ?c0 ) ) ?c0 )" if crate::trs::is_const_pos("?c0")),

        // INCONSISTENT
        // rw!("add-denom-div" ; "(/ (+ ?a (* ?b ?c)) ?b)"     => "(+ (/ ?a ?b) ?c)" if crate::trs::is_not_zero("?b")),
        // rw!("add-denom-mul" ; "(+ (/ ?a ?b) ?c)"            => "(/ (+ ?a (* ?b ?c)) ?b)" if crate::trs::is_not_zero("?b")),
        // rw!("add-div-mod"   ; "( + ( / ?x num2 ) ( % ?x num2 ) )" => "( / ( + ?x num1 ) num2 )"),
    ]
}

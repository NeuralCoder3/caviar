...
pub fn min() -> Vec<Rewrite> {
    vec![
        ...
        rw!("min-max-min-y2" ; "(min (max ?x ?z) ?y)" => "(min (max (min ?x ?y) ?z) ?y)"),
        rw!("min-sub-both"  ; "(min (+ ?a ?b) ?c)"                  => "(+ (min ?b (- ?c ?a)) ?a)"),
        rw!("min-sub-both2"  ; "(+ (min ?b (- ?c ?a)) ?a)" => "(min (+ ?a ?b) ?c)"),
        ...
        rw!("min-consts-or2"          ; "( || ( < ?y ?c1 ) ( < ?c0 ?c1 ) )" => "( < ( min ?y ?c0 ) ?c1 )"),
        rw!("max-consts-and"         ; "( < ( max ?y ?c0 ) ?c1 )" => "( && ( < ?y ?c1 ) ( < ?c0 ?c1 ) )"),
        rw!("max-consts-and2"         ; "( && ( < ?y ?c1 ) ( < ?c0 ?c1 ) )" => "( < ( max ?y ?c0 ) ?c1 )"),

...
pub fn min() -> Vec<Rewrite> {
    vec![
        ...
        rw!("min-sub-both"  ; "(+ (min ?b (- ?c ?a)) ?a)" => "(min (+ ?a ?b) ?c)"),
        rw!("min-add-both"  ; "(min (+ ?x ?z) (+ ?y ?z))" => "(+ (min ?x ?y) ?z)"),
        ...
        //FOLD
        rw!("min-consts-or"          ; "( || ( < ?y ?c1 ) ( < ?c0 ?c1 ) )" => "( < ( min ?y ?c0 ) ?c1 )"),
        rw!("max-consts-and"         ; "( && ( < ?y ?c1 ) ( < ?c0 ?c1 ) )" => "( < ( max ?y ?c0 ) ?c1 )"),
        rw!("max-consts-or"          ; "( || ( < ?c1 ?y ) ( < ?c1 ?c0 ) )" => "( < ?c1 ( max ?y ?c0 ) )"),
        rw!("min-consts-div-pos"     ; "( * ( min ?x ( / ?b ?a ) ) ?a )" => "( min ( * ?x ?a ) ?b )" if crate::trs::compare_c0_c1("?b", "?a", "%0<") ), // b%a==0 && 0<b        
        rw!("min-min-div-pos"        ; "( * ( min ?x ( * ?y ( / ?b ?a ) ) ) ?a )" => "( min ( * ?x ?a ) ( * ?y ?b ) )" if crate::trs::compare_c0_c1("?b", "?a", "%0<") ),  
        rw!("min-consts-div-neg"     ; "( * ( max ?x ( / ?b ?a ) ) ?a )" => "( min ( * ?x ?a ) ?b )" if crate::trs::compare_c0_c1("?b", "?a", "%0>") ),  
        rw!("min-min-div-neg"        ; "( * ( max ?x ( * ?y ( / ?b ?a ) ) ) ?a )" => "( min ( * ?x ?a ) ( * ?y ?b ) )" if crate::trs::compare_c0_c1("?b", "?a", "%0>") ), 

...
pub fn and() -> Vec<Rewrite> {
    vec![
        ...
        rw!("and-x-1"           ;  "(&& num1 ?x)"                          => "?x"),
        rw!("and-x-x"           ;  "(&& ?x ?x)"                         => "?x"),
        rw!("and-x-not-x"       ;  "(&& ?x (! ?x))"                     => "num0"),
        rw!("and-eq-eq"         ;  "( && ( == ?x ?c0 ) ( == ?x ?c1 ) )" => "num0" if crate::trs::compare_c0_c1("?c1", "?c0", "!=")),
        ...
        rw!("and-lt-gt-to-0"    ; "( && ( < ?c0 ?x ) ( < ?x ?c1 ) )"    => "num0" if crate::trs::compare_c0_c1("?c1", "?c0", "<=+1")),
        rw!("and-eqlt-eqgt-to-0"; "( && ( <= ?c0 ?x ) ( <= ?x ?c1 ) )"  => "num0" if crate::trs::compare_c0_c1("?c1", "?c0", "<")),
        rw!("and-eqlt-gt-to-0"  ; "( && ( <= ?c0 ?x ) ( < ?x ?c1 ) )"   => "num0" if crate::trs::compare_c0_c1("?c1", "?c0", "<=")),
    ]
}

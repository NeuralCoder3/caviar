...
pub fn lt() -> Vec<Rewrite> {
    vec![
        // LT RULES
        rw!("gt-to-lt"      ;  "(> ?x ?z)"              => "(< ?z ?x)"),
        rw!("gt-to-lt2"      ; "(< ?z ?x)" => "(> ?x ?z)"),
        rw!("lt-swap"      ;  "(< ?x ?y)"              => "(< (* -1 ?y) (* -1 ?x))"),
        rw!("lt-swap2"      ; "(< (* -1 ?y) (* -1 ?x))" => "(< ?x ?y)"),
        rw!("lt-to-zero"    ;  "(< ?a ?a)"              => "0"),
        rw!("lt-swap-in"    ;  "(< (+ ?x ?y) ?z)"       => "(< ?x (- ?z ?y))" ),
        rw!("lt-swap-in2"    ; "(< ?x (- ?z ?y))" => "(< (+ ?x ?y) ?z)" ),
        rw!("lt-swap-out"   ;  "(< ?z (+ ?x ?y))"       => "(< (- ?z ?y) ?x)" ),
        rw!("lt-swap-out2"   ; "(< (- ?z ?y) ?x)" => "(< ?z (+ ?x ?y))" ),
        ...
        rw!("min-lt-cancel2" ; "( < ?y ?x )" => "( < ( min ?x ?y ) ?x )"),
        rw!("lt-min-mutual-term"    ; "( < ( min ?z ?y ) ( min ?x ?y ) )"           => "( < ?z ( min ?x ?y ) )"),
        rw!("lt-min-mutual-term2"    ; "( < ?z ( min ?x ?y ) )" => "( < ( min ?z ?y ) ( min ?x ?y ) )"),
        rw!("lt-max-mutual-term"    ; "( < ( max ?z ?y ) ( max ?x ?y ) )"           => "( < ( max ?z ?y ) ?x )"),
        rw!("lt-max-mutual-term2"    ; "( < ( max ?z ?y ) ?x )" => "( < ( max ?z ?y ) ( max ?x ?y ) )"),
        ...
        // rw!("lt-max-term-term+pos2"  ; "( < ( max ?z ( + ?y ?c0 ) ) ?x )" => "( < ( max ?z ( + ?y ?c0 ) ) ( max ?x ?y ) )" if crate::trs::is_const_pos("?c0")),
        rw!("lt-min-term+neg-term"  ; "( < ( min ?z ( + ?y ?c0 ) ) ( min ?x ?y ) )" => "( < ( min ?z ( + ?y ?c0 ) ) ?x )" if crate::trs::is_const_neg("?c0")),
        // rw!("lt-min-term+neg-term2"  ; "( < ( min ?z ( + ?y ?c0 ) ) ?x )" => "( < ( min ?z ( + ?y ?c0 ) ) ( min ?x ?y ) )" if crate::trs::is_const_neg("?c0")),

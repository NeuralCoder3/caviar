...
pub fn lt() -> Vec<Rewrite> {
    vec![
        ...
        rw!("lt-to-zero"    ;  "(< ?a ?a)"              => "num0"),
        rw!("lt-swap-in"    ;  "(< (+ ?x ?y) ?z)"       => "(< ?x (- ?z ?y))" ),
        rw!("lt-swap-out"   ;  "(< ?z (+ ?x ?y))"       => "(< (- ?z ?y) ?x)" ),
        rw!("lt-x-x-sub-a"  ;  "(< (- ?a ?y) ?a )"      => "num1" if crate::trs::is_const_pos("?y")),
        rw!("lt-const-pos"  ;  "(< num0 ?y )"              => "num1" if crate::trs::is_const_pos("?y")),
        rw!("lt-const-neg"  ;  "(< ?y num0 )"              => "num1" if crate::trs::is_const_neg("?y")),
        ...
        rw!("lt-min-term+cpos"      ; "( < ( min ?x ?y ) (+ ?x ?c0) )"              => "num1" if crate::trs::is_const_pos("?c0")),
        rw!("lt-min-max-cancel"     ; "(< (max ?a ?c) (min ?a ?b))"                 => "num0"),
        // rw!("lt-mul-pos-cancel"     ; "(< (* ?x ?y) ?z)"                            => "(< ?x (/ ?z ?y))"  if crate::trs::is_const_pos("?y")),
        // rw!("lt-mul-div-cancel"     ; "(< ?x (/ ?z ?y))"                            => "(< (* ?x ?y) ?z))"  if crate::trs::is_const_pos("?y")),
        rw!("lt-const-mod"     ; "(< ?a (% ?x ?b))" => "num1"  if crate::trs::compare_c0_c1("?a", "?b", "<=-a")),
        rw!("lt-const-mod-false"     ; "(< ?a (% ?x ?b))" => "num0"  if crate::trs::compare_c0_c1("?a", "?b", ">=a")),
